...
func Client() {
	...
	case 14: //14.根据哈希查询一条交易
		...
		fmt.Println("请输入Hash：")
		fmt.Scanln(&s.Hash)
		rpc2, err := jsonrpc.Dial("tcp", "127.0.0.1:8010")
		if err != nil {
			log.Fatal(err)
		}
		rpc3, err := jsonrpc.Dial("tcp", "127.0.0.1:9010")
		if err != nil {
			log.Fatal(err)
		}

		var basic2,basic3 BasicMessage
		err2 := rpc2.Call("PPoV.GetTransactionByHash", s, &basic)
		if err2 != nil {
			log.Fatal(err2)
		}

		err3 := rpc3.Call("PPoV.GetTransactionByHash", s, &basic2)
		if err3 != nil {
			log.Fatal(err3)
		}

		err4 := rpc1.Call("PPoV.GetTransactionByHash", s, &basic3)
		if err4 != nil {
			log.Fatal(err2)
		}

		data, _ := json.Marshal(basic)
		data1,_ := json.Marshal(basic2)
		data2,_ := json.Marshal(basic3)

		if string(data) == string(data1) && string(data1) == string(data2){
			fmt.Println(string(data))
			fmt.Println(string(data1))
			fmt.Println(string(data2))
		}else{
			fmt.Println("minic nodes verify failed ")
			fmt.Println(string(data))
			fmt.Println(string(data1))
			fmt.Println(string(data2))
		}
	...
	case 17: //17.创建账户
		rpc2, err := jsonrpc.Dial("tcp", "127.0.0.1:9010")
		if err != nil {
			log.Fatal(err)
		}
		rpc3, err := jsonrpc.Dial("tcp", "127.0.0.1:10010")
		if err != nil {
			log.Fatal(err)
		}
		...
		timestamp,err :=strconv.Atoi(s.Timestamp)
		if timestamp % 3 == 0{
			...
			err2 := rpc2.Call("PPoV.CreatAccount", s, &basic)
			if err2 != nil {
				log.Fatal(err2)
			}
		} else if timestamp % 3 == 2{
			err2 := rpc3.Call("PPoV.CreatAccount", s, &basic)
			if err2 != nil {
				log.Fatal(err2)
			}
		}else {
	...
	case 19: //19.查询余额
		...
		var basic2,basic3 BasicMessage
		err2 := rpc2.Call("PPoV.GetBalance", s, &basic)
		if err2 != nil {
			log.Fatal(err2)
		}

		err3 := rpc3.Call("PPoV.GetBalance", s, &basic2)
		if err3 != nil {
			log.Fatal(err3)
		}

		err4 := rpc1.Call("PPoV.GetBalance", s, &basic3)
		if err4 != nil {
			log.Fatal(err2)
		}
