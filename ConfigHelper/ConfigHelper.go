...
func CreateConfigs(){
	...
	var servers=[]AddressPair{
		AddressPair{"10.0.0.19", 5010},
		AddressPair{"10.0.0.20", 5010},
		AddressPair{"10.0.0.21", 5010},
		AddressPair{"10.0.0.22", 5010}}
	conf.MinicNum = 0
...
func (conf Config) WriteFile(file string) {
	...
	c.AddOption("network","MinicNum", strconv.Itoa(conf.MinicNum))
	//设置IP
...
func (conf *Config) ReadFile(file string) {
	...
	conf.MinicNum, err = c.Int("network", "MinicNum")
	...
	//读取MinicList
	str, err = c.String("network", "MinicList")
	str_lists := strings.Split(str, ";")
	minicList := [][]AddressPair{}
	for _, str_list := range str_lists {
		list := []AddressPair{}
		str_nodes = strings.Split(str_list, ",")
		for _, str_node := range str_nodes {
			var addr AddressPair
			split_str_node := strings.Split(str_node, ":")
			addr.IP = split_str_node[0]
		...
		minicList = append(minicList, list)
	}
	minicNumList := []int{}
	str, err = c.String("network", "MinicNumList")
	str_nodes = strings.Split(str, ";")
	for _, str_node := range str_nodes{
		num,_ := strconv.Atoi(str_node)
		minicNumList = append(minicNumList, num)
	}

	if len(minicList) != len(minicNumList){
		panic("the length of minicList isn't equal to the length of minicNumList")
	}
	conf.MinicList = make(map[int][]AddressPair)
	for i := 0; i < len(minicNumList); i++ {
		conf.MinicList[minicNumList[i]] = minicList[i]
	}
