...
type Network struct {
	...
	MinicList   map[int]map[NodeID]NodeInfo
	ServicePort int
...
func (network *Network) SetConfig(config ConfigHelper.Config) {
	...
	network.MinicList = make(map[int]map[NodeID]NodeInfo)
	for i, list0 := range config.MinicList {
		tmp := make(map[NodeID]NodeInfo)
		for _, x := range list0{
			temp := GetNodeId(x.IP, x.Port)
			_, ok := tmp[temp]
			if !ok{
