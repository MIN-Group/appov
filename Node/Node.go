		...
		if item.ReceivedBlockGroupHeader {
			...
			if flag {
...

				{
					...
					if duty_pubkey == pubkey {
						item.ExecutionResult = make(map[string]bool)
						headermsg, blockmsg := node.msgManager.CreatePublishBlockGroupMsg(0, height, node.config.MinicNum)
						blockmsg.Group = item
						node.SendMessageToMinic(headermsg, &blockmsg)
					}
				}
...
func (node *Node) SendMessageToMinic(header Message.MessageHeader, messageInterface Message.MessageInterface) {
	data, err := messageInterface.ToByteArray()
	if err != nil {
		fmt.Println("SendMessageToMinic：messageInterface.ToByteArray()错误！")
	}
	header.Data = data
	b, err := header.MarshalMsg(nil)
	if err != nil {
		fmt.Println("SendMessagToMinice：header.MarshalMsg()错误！")
	}
	go node.network.SendToAllMinic(b)
}
...
func (node *Node) HandleMessage(data []byte, conn net.Conn) {
	...
	case Message.PublishBlockGroup:
...
func (node *Node) HandlePublishBlockGroup(msg Message.PublishBlockGroupMsg) {
	...
	sum := 0
	for _, value := range group.ExecutionResult {
		if value == true {
			sum++
		}
	}
...
func (node *Node) HandleGenesisBlockPublishMessage(msg Message.BlockMsg, header Message.MessageHeader) {
	...
	temp_hash := KeyManager.GetHash(temp)
	ok, err := node.keymanager.Verify(temp_hash, sig, bg.Generator)
	if err != nil {
		fmt.Println(err)
	}
	if ok {
...
func (node *Node) HandleBlockGroupHeaderMessage(msg Message.BlockGroupHeader, header Message.MessageHeader) {
	...
	temp_hash := KeyManager.GetHash(temp)
	ok, err := node.keymanager.Verify(temp_hash, blockgroup_header.Sig, blockgroup_header.Generator)
	if err != nil {
		fmt.Println(err)
	}
	if ok {
